...
#
# Renames the RN *component name* (PearStarter -> your name) and the Android
# *application id* (com.pearstarter -> your id), including the package-dir move.
# It does NOT touch:
...
OLD_NAME="PearStarter"
OLD_DIR="app/android/app/src/main/java/com/pearstarter"
...
perl -pi -e "s/withModuleName: \"\Q$OLD_NAME\E\"/withModuleName: \"$NEW_NAME\"/"                     app/ios/PearStarter/AppDelegate.swift
perl -pi -e "s/rootProject.name = '\Q$OLD_NAME\E'/rootProject.name = '$NEW_NAME'/"                  app/android/settings.gradle

echo "→ android app id: com.pearstarter → $NEW_ID (applicationId + namespace + package dir)"
perl -pi -e "s/applicationId \"com\.pearstarter\"/applicationId \"$NEW_ID\"/"                            app/android/app/build.gradle
perl -pi -e "s/namespace \"com\.pearstarter\"/namespace \"$NEW_ID\"/"                                    app/android/app/build.gradle
mkdir -p "$(dirname "$NEW_DIR")"
git mv "$OLD_DIR" "$NEW_DIR" 2>/dev/null || mv "$OLD_DIR" "$NEW_DIR"
perl -pi -e "s/^package com\.pearstarter\b/package $NEW_ID/" "$NEW_DIR"/*.kt
...
Axis 1 done. STILL MANUAL (see README → "Renaming for your app"):
  • iOS: rename the target/scheme/folder inside Xcode, update the Podfile target +
    'pod ...', set your real bundle id (now com.pearstarter, in project.pbxproj
  ...
  • Axis 2 (optional): com.margelo.nitro.pearstarter + PearStarter* module names — re-run
